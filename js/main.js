...
function openMenMenu() {
 ...
 window.onclick = function(event) {
    ...
    if (!event.target.matches('.Menbtn') && !event.target.matches('.Femalebtn') && !event.target.matches('.Childbtn') && !event.target.matches('.Beautybtn') && !event.target.matches('.Homebtn')) {
        // Hämta alla element med klasserna .navbar-2, .herr-meny och .Dam-meny
        var dropdowns = document.querySelectorAll(".navbar-2, .herr-meny, .Dam-meny, .barn-meny, .beauty-meny, .home-meny");
        var i;
